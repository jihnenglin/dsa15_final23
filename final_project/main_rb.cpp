...
void inorder_wild(const struct rb_node *node, vector<Account *>* v, const string& wild, Account* current){
	if(node == NULL) return;
	if(node->rb_link[0] != NULL)
		inorder_wild(node->rb_link[0], v, wild, current);
	if(match_wild(((Account *)node->rb_data)->id, wild) && (Account *)node->rb_data != current)
...
int main(){
	...
	while(scanf("%s", request) != EOF){
		if(strcmp(request, "login") == 0){
			...
			inorder_wild(tree->rb_root, wild, string(id1), current);
			if(wild->size() != 0){
				cout << (*wild)[0]->id;
